...
| `cocotb_tb/top/` | End-to-end testbench driving the real AXI4-Lite host MMIO interface -- see its own README | done (M4) |
...
| `sva/` | SystemVerilog Assertions, bound into the DUT via `bind` (not embedded in `rtl/`); `sram_sva.sv`, `matrix_engine_sva.sv`, `dma_sva.sv`, `cmd_proc_sva.sv` done | added per block's milestone |
| `coverage/` | Functional/cross/FSM covergroups; `sram_cov.sv`, `matrix_engine_cov.sv`, `dma_cov.sv`, `cmd_proc_cov.sv` (incl. FSM state/arc coverage) done | added per block's milestone |
