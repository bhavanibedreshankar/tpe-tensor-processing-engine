...
| `testlists/` | `standalone.yaml`/`sanity.yaml`/`smoke.yaml` (hand-curated) + `daily.yaml`/`random.yaml` (generated by `tools/gen_tests.py`) -- test tier definitions consumed by `tools/regression.py` | done (M6) |

See [`docs/verification/test_plan.md`](../docs/verification/test_plan.md)
for methodology and per-tier sizing,
[`docs/verification/coverage_plan.md`](../docs/verification/coverage_plan.md)
for how coverage is modeled and merged, and
[`docs/flows/regression_flow.md`](../docs/flows/regression_flow.md) for how
to run/reproduce/profile any regression tier.
