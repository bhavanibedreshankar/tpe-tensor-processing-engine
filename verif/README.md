...
| `cocotb_tb/env/` | Reusable pyuvm classes: `SyncPortItem/Driver/Monitor/Agent`, `RowCollector`, `TpeBaseTest`, `golden_model.run_tpe_model` | done (M1, extended M3) |
...
| `sva/` | SystemVerilog Assertions, bound into the DUT via `bind` (not embedded in `rtl/`); `sram_sva.sv`, `matrix_engine_sva.sv`, `dma_sva.sv` done | added per block's milestone |
| `coverage/` | Functional/cross/FSM covergroups; `sram_cov.sv`, `matrix_engine_cov.sv`, `dma_cov.sv` (incl. FSM state/arc coverage) done | added per block's milestone |
