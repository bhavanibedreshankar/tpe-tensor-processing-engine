...
defect) -- added so the failure *type* itself, not just the message text,
tells you which category of bug caught a test: a scoreboard's data-compare
mismatch raises `MismatchError`, a single status/register check uses
`pyuvm.uvm_error()`/`uvm_fatal()` (raising `UVMError`/`UVMFatalError`), and
`tpe_model` itself failing outright (as opposed to its output mismatching
RTL) raises `CModelError` (`verif/cocotb_tb/env/errors.py`,
`verif/cocotb_tb/env/golden_model.py`). `run_sim`'s status/monitor/summary
tables and its `FAILURE SIGNATURE` column
(`docs/flows/run_sim_flow.md` section 10) show whichever of these actually
fired, not a generic `AssertionError`.
